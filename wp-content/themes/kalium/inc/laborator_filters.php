...
function shortcode_lab_social_networks( $atts = array(), $content = '' ) {
	...
	$social_order_list = array(
		...
		"xi"  => array("title" => __( "Xing", 'kalium' ), 			"icon" => "fa fa-xing"),
		"sp"  => array("title" => __( "Spotify", 'kalium' ), 		"icon" => "fa fa-spotify"),
	...
	foreach ( $social_order['visible'] as $key => $title ) {
		...
		$html .= '<li>';
			$html .= '<a href="' . $href . '" target="' . $link_target . '" class="color '.( preg_match( "/^[0-9]+/", $class ) ? 'social-' : '' ) . $class . '" title="' . $title . '">';
				...
				$html .= '<span class="name">' . apply_filters( 'lab_social_networks_name' , $title_span, $title ) . '</span>';
			$html .= '</a>';
		$html .= '</li>';
	}
...
function laborator_header_spacing() {
	...
	if ( is_singular() ) {
		...
		// Footer Visibility
		$footer_visibility = get_field( 'footer_visibility' );
		...
		// Fixed Footer
		$fixed_footer = get_field( 'fixed_footer' );
	...
	} else if ( is_home() && get_queried_object() instanceof WP_Post ) {
...
function kalium_wp_video_shortcode_output( $html, $atts, $video, $post_id, $library ) {
	...
	return kalium_video_aspect_radio_holder( $html, $atts['width'], $atts['height'] );
}
...
?><video class="<?php echo apply_filters( 'wp_video_shortcode_class', '' ); ?>" controls <?php echo trim( $video_atts_html ); ?>></video><?php
	...
	return kalium_video_aspect_radio_holder( $return, $width, $height, $maintain_aspect_ratio );
}
...
function kalium_video_aspect_radio_holder( $html, $width = 0, $height = 0, $enabled = true ) {
	
	if ( is_numeric( $width ) && is_numeric( $height ) && $width > 0 ) {
		$uniqueid = lab_unique_id();
...
function kalium_use_filebased_custom_skin_filter( $use ) {
	...
	if ( $use ) {
		if ( is_child_theme() ) {
			wp_enqueue_style( 'custom-skin', get_stylesheet_directory_uri() . '/custom-skin.css', null, $theme_version );
		} else {
			wp_enqueue_style( 'custom-skin', get_stylesheet_directory_uri() . '/assets/css/custom-skin.css', null, $theme_version );
		}
	}
}


// Portfolio Lightbox Navigation Mode - Single
function kalium_lightbox_single_navigation_mode_class( $classes ) {
	$classes[] = 'kalium-lb-single-nav-mode';
	return $classes;
}


// Navigation mode for Lightbox Items
add_filter( 'template_include', 'kalium_lightbox_single_navigation_mode_check' );

function kalium_lightbox_single_navigation_mode_check( $template ) {
	
	if ( in_array( basename( $template ), array( 'archive-portfolio.php', 'portfolio.php' ) ) && kalium_lb_get_navigation_mode() == 'single' ) {
		add_filter( 'body_class', 'kalium_lightbox_single_navigation_mode_class' );
	}
